...
public class Terminal implements CommandLineRunner {
    ...
    @Autowired
    public Terminal(UserService userService, GameService gameService, OrderService orderService) {
    ...
    private void tryEditGame(String[] tokens) {
        ...
        if (!this.currentlyLoggedUser.isAdmin()) {
            System.out.println("You are not authorized to do that!");
            return;
        }
        Long id = Long.parseLong(tokens[1]);
        ...
        for (int i = 2; i < tokens.length; i++) {
            ...
            switch (values[0]) {
                case "price":
                    ...
                    break;
                case "size":
                    game.setSize(new BigDecimal(values[1]));
                    break;
                case "description":
    ...
    private void tryAddGame(String[] tokens) throws ParseException {
        ...
        try {
            ...
            game.setSize(new BigDecimal(tokens[3]));
            game.setTrailer(tokens[4]);
            game.setImageURL(tokens[5]);
